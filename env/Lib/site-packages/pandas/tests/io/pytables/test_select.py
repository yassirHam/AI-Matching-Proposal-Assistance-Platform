...
def test_select_with_many_inputs(setup_path):
    with ensure_clean_store(setup_path) as store:
        ...
        _maybe_remove(store, "df")
        store.append("df", df, data_columns=["ts", "A", "B", "models"])
        ...
        # small selector
        result = store.select("df", "ts>=Timestamp('2012-02-01') & models=['a','b','c']")
        expected = df[
        ...
        result = store.select("df", "ts>=Timestamp('2012-02-01') and models=selector")
