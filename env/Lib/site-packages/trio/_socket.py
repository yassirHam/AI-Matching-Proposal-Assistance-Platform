...
# has ended up typing those return types as `Any` in most cases, but for models that
...
# most models, so currently we just specify it as `Any`. Otherwise we would write:
